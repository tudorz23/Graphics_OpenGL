...
Mesh* objects3d::CreateCone(const std::string& name, float radius, float height,
							int num_slices, glm::vec3 color)
{
    std::vector<VertexFormat> vertices;
    std::vector<unsigned int> indices;

    float tip_y = height * 3.0f / 4.0f;
    float base_y = -height / 4.0f;

    // Add vertex for center of base.
    vertices.emplace_back(glm::vec3(0, base_y, 0), color);

    // Add vertex for tip.
    vertices.emplace_back(glm::vec3(0, tip_y, 0), color);
    ...
    for (int i = 0; i < num_slices; i++) {
        ...
        vertices.emplace_back(glm::vec3(x, base_y, z), color);
    }
